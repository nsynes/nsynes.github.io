(function (window, undefined) {
    var MapsLib = function (options) {
        ...
        // Important! this key is for d	emonstration purposes. please register your own.
        ...
        //-----custom initializers-----
		// max and min allowable values for the date slider
		var minDate = moment("Jan 1 2010"); // Jan 1st 2010
		var maxDate = moment(); //now

		// starting values
		var startDate = minDate; //moment().subtract('months', 36); //past 3 months
  ...
  MapsLib.prototype.sliderInterval = function(interval) {
    ...
    MapsLib.prototype.doSearch = function () {
		console.log("search function has been run");
        var self = this;
        self.clearSearch();
        ...
        //-----custom filters-----
		self.whereClause += " AND 'ActivityDate' >= '" + $('#startDate').html() + "'";
		self.whereClause += " AND 'ActivityDate' <= '" + $('#endDate').html() + "'";

		if ( $("#select_type").val() != "")
			  self.whereClause += " AND 'ActivityType' = '" + $("#select_type").val() + "'";
